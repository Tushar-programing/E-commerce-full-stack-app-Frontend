  ...
  const fetchCartData = async (start, end) => {
    try {
      ...
      const response = await axios.post(
        `${conf.apiUrl}/users/getAllUserData`,
        {
          startDate: start,
          endDate: end,
        },
    ...
    if (query) {
      const filtered = cartData.filter(
        (item) =>
          ...
          item.fullName?.toString().includes(query)
      );
          ...
          {filteredData.map((cart, index) => (
            <React.Fragment key={index}>
              <div className="col-span-4 p-2 border-b border-r border-gray-300">
                <div>ID: {cart?._id || "N/A"}</div>
                <div>Name: {cart?.fullName || "N/A"}</div>
              </div>
              ...
              >
                {cart?.mobile && <div>Mobile: {cart?.mobile || "N/A"}</div>}
                {cart?.email && <div>Email: {cart?.email || "N/A"}</div>}
              </Link>
              ...
              <div className="col-span-2 p-2 border-b border-l border-gray-300">
                <button
                  onClick={() =>
                    axios.post(conf.apiUrl + "export-json", cart.mobile)
                  }
                >
                  Export
                </button>
