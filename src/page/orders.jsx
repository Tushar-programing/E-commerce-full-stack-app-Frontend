...
function Orders() {
  const [payment, setPayment] = useState(true);
    ...
    const checkhandle = () => {
      ...
      const create = async() => {
        if (adress.length > 0) {
          await axios.post(`${conf.apiUrl}/order/createOrder/${slug}`, {quantity: quantity? quantity : 1, adress: selectedOption? selectedOption : adress[0]._id, status: "confirm", paymentStatus: option === 50 ? "delivery" : option === 80? "post office" : "COD"}, {
          withCredentials: true
          }).then((order) => {
          console.log(order);
